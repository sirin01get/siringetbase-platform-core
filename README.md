...
- Comms (`src/lib/comms/`, design: `../comms/README.md`): `EmailSenderPort` + `ResendAdapter`, a (vertical, role, triggerEvent) template registry (`templates/registry.ts`, CA Focus's magic-link copy in `templates/ca.ts`, a role-agnostic fallback in `templates/fallback.ts`), Standard Webhooks HMAC verification (`verify-webhook.ts`), and the `notification_dispatch` delivery log (`0006_notification_dispatch.sql`). Two entry points converge on all of this: `POST /api/comms/auth-email-hook` (Supabase's Send Email Hook target — auth-lifecycle email) and `sendNotification()` (`send-notification.ts` — everything else, not yet called from any route). **Built but not activated** — see `../comms/README.md`'s Rollout Plan step 2 for the exact dashboard steps (hook registration, Worker Secrets, Resend domain verification) still needed before a real email goes out through it.
- **Not real yet**: `../billing/`'s subscription/cost-plus/revenue-share tables (a follow-up migration), any real gateway/bank integration (mocks only, by design — see `../payments/README.md`), comms's non-auth sends (marketer-invite/verification-decision emails — `sendNotification()` exists but nothing calls it yet), and no product UI beyond the placeholder status page and admin sync-queue view above (siringetbase owns no *product* screens — see `../design-system/README.md`).
