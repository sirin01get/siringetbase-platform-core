...
- **Not real yet**: `../billing/`'s subscription/cost-plus/revenue-share tables (a follow-up migration), any real gateway/bank integration (mocks only, by design — see `../payments/README.md`), and no product UI beyond the placeholder status page and admin sync-queue view above (siringetbase owns no *product* screens — see `../design-system/README.md`).
