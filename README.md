...
- Chat + feature requests (`0035_chat_and_feature_requests.sql`): `chat_threads`/`chat_messages` (the WhatsApp-patterned two-thread-per-user support channel) and `feature_requests` (a dedup-free sibling of `support_error_reports`, filed from the same capture flow but for feature asks instead of bugs) — this project owns the shared tables plus `templates/support.ts`'s `support.feature_request_filed` template, same split as Support Escalation above. All chat/feature-request service logic, API routes, and UI live in `../../cafocus/app`.
...
- Env-check precheck dashboard (`src/lib/admin/env-check.ts`, `/admin/env-check`, `app/api/internal/env-check/route.ts`): every env var this project reads via `src/config/env.ts`'s `required()` helper, listed with a presence/shape check (set vs. missing, plausible URL vs. placeholder) — not a live-connectivity check, that's still `/api/diagnostics`'s job. Built after `PLATFORM_CORE_BASE_URL` twice went missing from `../../cafocus/app`'s Cloudflare Worker without anyone noticing until a dependent feature broke (see Troubleshooting below). `GET /api/internal/env-check` exposes this project's own report cross-Worker, secret-header-protected via `ENV_CHECK_INTERNAL_SECRET` (same "separate secret per internal surface" posture as every other internal secret above), so `../../cafocus/app`'s own `/admin/env-check` can render both apps' reports side by side plus a shared-secret fingerprint match (SHA-256, truncated — never the actual secret value) in one combined view. `business_admin`/`support_admin` only, same as every other admin page.
...
### Cloudflare env vars: a plain Variable can silently disappear across redeploys — use a Secret instead

Symptom: an env var that was set and working (e.g. `PLATFORM_CORE_BASE_URL`) goes missing after a later, unrelated push — no one touched Settings, but the next build fails `required()` for that var. This project's `wrangler.jsonc` has no `vars` block, so it isn't the classic "wrangler.toml overwrites dashboard vars on every deploy" gotcha; the more likely cause is Cloudflare Workers Builds' own Git-integrated CI creating a **fresh Worker version on every push**. Community reports of Cloudflare Workers Builds describe plain **Variables** (Settings → Variables & Secrets, stored as text) as sometimes failing to carry over onto a new version this way, while true **Secrets** (same screen, encrypted) reliably persist, because a Secret is stored independently of any specific Worker version rather than copied per-version.

If a var keeps reappearing-then-vanishing with no code change to explain it, re-add it as a Secret (Settings → Variables & Secrets → Edit → toggle "Encrypt"), not a plain Variable, even if its value isn't actually sensitive — it's the storage mechanism that's more durable here, not a security requirement. Use `/admin/env-check` (see "What's real in this phase" above) to confirm presence after every deploy rather than waiting for a dependent feature to fail first.
...
**Deploys run through Cloudflare Workers Builds' Git integration (Cloudflare's own Linux build infra), not a local `npm run deploy` on the owner's machine** — see the "Windows + Node 24 local deploy is unsupported" note below for why. Because of that, `wrangler` is floated at `^4.96.0` rather than pinned to an exact version: `wrangler@4.86.0` through at least `4.95.x` crash on Windows + Node 24 with `spawn UNKNOWN` (this Worker's `AI` binding forces wrangler to spin up a local remote-bindings proxy on every `build`/`deploy`/plain `wrangler deploy`, which is exactly the code path that regressed; see [cloudflare/workers-sdk#14054](https://github.com/cloudflare/workers-sdk/issues/14054), closed "not planned" with no confirmed fix version as of this writing). `^4.96.0` both clears that broken range and satisfies `@opennextjs/cloudflare`'s own `peerDependency` on `wrangler@^4.86.0` — an exact `4.85.0` pin (this project's original workaround) undershoots that peer requirement and breaks `npm ci`/`npm install` outright once `@opennextjs/cloudflare` is bumped past whatever version first introduced the `^4.86.0` peer floor. If you ever do need to run `npm run deploy` locally on Windows + Node 24 (not the supported path — see below), first confirm whatever `wrangler` version resolves is still outside the 4.86–4.95.x band.

**Windows + Node 24 local deploy is unsupported for this project — use Cloudflare Workers Builds' Git integration instead.** Beyond the wrangler bug above, this Worker's `AI` binding has no local simulation, so `getPlatformProxy()` spawns a real `workerd` runtime process even just to resolve bindings during `deploy`/`preview` — and that `workerd` binary itself has been observed to crash on Windows with `std::terminate() called with no exception` / `MiniflareCoreError [ERR_RUNTIME_FAILURE]`, a binary-level incompatibility with no version-pin workaround. `npm run preview`/`npm run deploy` are fine on Linux/macOS/WSL2 (OpenNext's own Windows-native support is explicitly "may work, WSL2 recommended" regardless), but the actual supported path for shipping this project from Windows is: push to `main` (or trigger an empty commit — `git commit --allow-empty -m "..."`) and let Cloudflare's Git-integrated build run `npm run cf:build` on its own Linux infra, never local `wrangler`/`workerd`.
...
### Activating `GET /api/internal/env-check` (env-check dashboard's cross-Worker read)

Same shared-secret pattern as the other internal endpoints above:

1. Set `ENV_CHECK_INTERNAL_SECRET` here (Settings → Variables & Secrets) to a long random value — set it as a **Secret**, not a plain Variable, per the Troubleshooting entry above.
2. Set the **identical** value as `ENV_CHECK_INTERNAL_SECRET` on `../../cafocus/app`'s deployment.
3. Visit `/admin/env-check` on **either** app — cafocus/app's combined view calls this endpoint to pull in platform-core's own report and compare the four shared secrets (`COMMS_INTERNAL_SECRET`, `DOCUMENT_INTELLIGENCE_INTERNAL_SECRET`, `PAYMENTS_INTERNAL_SECRET`, `SUBSCRIPTIONS_INTERNAL_SECRET`) by fingerprint; this project's own `/admin/env-check` only shows its own vars.

A mismatched/missing secret fails loudly (401, surfaced in the dashboard as "platform-core unreachable" rather than a raw error) — check both sides match exactly if the combined view can't reach platform-core.
